...
export const TvSetNavigatorX: React.FC<any> = ({
  children,
  noiseDuration = 20000,
  config,
  ...props
  ...
  const { channel: routeChannel } = router.query;
  console.log("MYLOG: routeChannel: ", routeChannel);

  const [channelMeta, setChannelMeta] = useState<channelMeta>({
    activeChannel: routeChannel ?? START_CHANNEL,
    prevChannel: routeChannel ?? START_CHANNEL,
    overlay: "off",
    infoOverlay: false,
    channelNumber: "fixed",
  });
  ...
  const touchToggleTemplate = () => {
    return globalTouchDetection ? (
      <div
        className="absolute  bg-gray-50 bottom-4 right-4  flex algin-center justify-center w-10 h-10 rounded-md"
        onClick={() => {
      ...
      <div
        className="absolute bg-gray-400 bottom-4 right-4 flex algin-center justify-center w-10 h-10 rounded-md"
        onClick={() => {
          setGlobalTouchDetection((prevState) => {
            return !prevState;
          });
